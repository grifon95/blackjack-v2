...
let randomNum1 = Math.floor(Math.random() * 14) + 1;
const card1 = document.createElement("img");
card1.setAttribute("src", `img/cards/${randomNum1}.png`);
divDealerCards.appendChild(card1);
const card2 = document.createElement("img");
card2.setAttribute("src", "img/cards/card-back.jpg");
divDealerCards.appendChild(card2);
cardsPulledBot.push(randomNum1);
...
if (randomNum1 > 11) {
  scoreDealerEl.textContent = 10;
} else if (randomNum1 === 1) {
  scoreDealerEl.textContent = 11;
} else if (randomNum1 === 11) {
  scoreDealerEl.textContent = 11;
} else {
  scoreDealerEl.textContent = randomNum1;
}
let isPlaying = true;
...
btnDeal.addEventListener("click", function () {
  ...
  isPlaying = true;
  randomNum1 = Math.floor(Math.random() * 14) + 1;
  const card1 = document.createElement("img");
  card1.setAttribute("src", `img/cards/${randomNum1}.png`);
  divDealerCards.appendChild(card1);
  const card2 = document.createElement("img");
...
btnStand.addEventListener("click", function () {
  if (isPlaying) {
    ...
    if (cardsPulled.length < 2) {
      ...
      // }
      for (let i = 0; i < cardsPulledBot.length; i++) {
        // scoreDealer += randomNum;
        // scoreDealerEl.textContent = scoreDealer;
        if (cardsPulledBot[i] > 11) {
          cardsPulledBot[i] = 10;
        }
        scoreDealer += cardsPulledBot[i];
        scoreDealerEl.textContent = scoreDealer;
      }
